...
# Pattern "**/*" recursively walk a directory for searching all files including folders
def sort_files(path: Path):
    
    for obj in path.glob("**/*"):
        flag = True
        # script to ignore these folders
        for i in ['images', 'video', 'documents', 'music', 'archives', 'unknown']:
            if i in str(obj):
                flag = False
                break
       # Changing cyrillic letters to latin letters and rename file
                ...
                if img_folder.exists():
                    ...
                    if file.name in lst_img:
                        ...
                        new_file = file.with_name(
                            file.stem + add_name).with_suffix(file.suffix)
...
# Delete all empty folders.Pattern "*/**" recursively walk a directoryfor searching only folders. With[::-1] start walk from deepest level
